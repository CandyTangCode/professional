---
layout: archive
title: "CV"
permalink: /cv/
author_profile: true
redirect_from:
  - /resume
---

{% include base_path %}

Education
======
* M.B.A University of Washington, 2025
* M.S. in Biomedical Engineering, Illinois Institute of Technology
* B.S. in Bioengineering, China Three Gorges University

Work experience
======
* Pathways Operations Manager, Amazon Logistics
  * Participate in the Pathway Leadership Program
  * Lead data-driven labor planning for 100+ associates daily, leveraging analytics to optimize real-time staffing decisions, resulting in balanced delivery performance and cost efficiency while maintaining safety standards
  * Tackle a long-standing operational bottleneck, cutting daily labor hour waste by 35.29% through targeted associate engagement (daily startup meetings, 1:1s, and consistent recognition)
  * Identify process defects and proposed barcode implementation, reducing vehicle safety inspection time by 25% while maintaining top safety standards

* Product Management Consultant, Stack Moxie (a B2B marketing SaaS startup focused on revenue operations)
  * Led customer interviews to validate senior leadership’s feature proposals; uncovered critical gaps and major clients' needs through data-backed insights; self-taught new tools (Figma and Stitch) to prototype a new user interface adopted by senior leadership
  * Delivered Amazon-style documentation (PRFAQ and white paper) and an executive-level presentation that shaped product direction and secured stakeholder alignment across engineering, customer success, and leadership teams

* Private Equity Intern, Brockhurst & Buchanan (a PE firm focused on senior care), Seattle, WA 2024
  * Led screening calls and interviewed CEOs of mid-market senior care companies; managed high-potential pipelines across multiple sectors and conducted due diligence, supporting $12M in raised capital

* Finance Intern, Nitecapp (a generative AI-powered tech startup in hospitality education), Seattle, WA 2024
  * Drove strategy and prioritization with diverse inputs to develop a pricing strategy through customer analysis, price sensitivity, WTP assessment, and benchmarking, projected to generate an additional $52M in revenue
  * Directed a cross-functional effort to design a pre-revenue valuation model based on comprehensive market research, risk assessment, multi-factor financial modeling, and distilling insights from expert interviews; unlocked hidden value creation opportunities for $8M

* Project Manager/Senior Research Associate, Allen Institute in Brain Science, Seattle, WA 2022 – 2023
  * Founded by Microsoft co-founder Paul Allen, a world-leading research institute in human brain science
  * Developed a brand-new efficiency evaluation for high-profile therapeutics; conducted feasibility and risk assessments, designed processes from scratch, and screened vendors and negotiated terms; improved sensitivity by 7x and reduced turnaround by 3x
  * Conceptualized (processes, people, systems) a homogenization system; led market research, best practice analysis, statistical modeling, and strategic fine-tuning; built top-notch assessment of drug efficiency, critical for all therapeutics in the pipeline (with $4.3B in value)

* Associate Scientist II, Arrowhead Pharmaceutical, Madison, WI 2019 – 2022
  * A pioneering RNAi pharma firm and strategic partner for Fortune 500 companies, No.1 in Deloitte's Technology Fast 500
  * Led safety evaluations for 25 high-profile drug candidates using a unique design of experiments, rigorous data analysis, and synthesis of insights; identified 8 safe candidates, contributing to securing $160M and potentially $1.57B in additional funding from 2 major clients
  * Drove a product design strategy for a next-generation drug platform by identifying capability gaps, leading extensive research, analyzing vast experimental data, and preparing recommendations; enabling the company to develop new drug types ($1.4B potential)
  * Created a user-friendly training platform through needs assessment, stakeholder interviews, and IT collaboration; enabled company-wide virtual learning, all with zero budget and zero dedicated resources

* Biologist II, Abbott Laboratories, Des Plaines, IL 2016 – 2019
  * A healthcare company, No.1 Most Admired Company in the medical products and equipment industry by Fortune Magazine
  * Architected multi-year verification studies for a new supplier; designed unique experiment programs, analyzed extensive datasets, synthesized findings, contributed to business and scientific cases for new materials; saved $2.5M/year due to adopting new supplies
  * Recognized by C-level exec for design and execution of exceptional training strategy to onboard scientists on a vital market ($3.2B)
  * Led cross-functional collaboration to resolve technical issues and enhance global customer satisfaction across 10 products
  * Managed a team of 3; pioneered a lab management strategy by streamlining procedures, resulting in a $1M cost reduction
  
Skills
======
🎯 Core Competencies
  * Product Management
  * Program Management
  * Operations Leadership
  * Strategic Planning & Execution
  * Cross-Functional Collaboration
  * Stakeholder Management
  * Process Improvement
  * Project Management

📊 Data & Analytical Skills
  * Data Analysis & Visualization
  * Statistical Modeling
  * Design of Experiments (DOE)
  * Forecasting & Performance Metrics
  * Financial Modeling & Valuation
  * Pricing Strategy & Revenue Analytics
  * Benchmarking & Market Research
  * Willingness-to-Pay (WTP) Assessment
  * SQL (Enthusiastic User)
  * Machine Learning (Certificate Holder)

💻 Technical Tools
  * SQL
  * Figma
  * Stitch
  * Microsoft Excel (Advanced)
  * Data Visualization Tools (e.g., dashboards, reports)
  * Amazon-Style Documentation (PRFAQ, White Papers)

💼 Business & Industry Expertise
  * SaaS Product Development
  * Operations & Supply Chain (Amazon Logistics)
  * Healthcare & Life Sciences (Abbott, Arrowhead, Allen Institute)
  * Financial Strategy & Private Equity
  * Biomedical Engineering
  * AI-Powered Tech Startups

🏆 Achievements & Recognition
  * 2× Meritorious Winner – International Mathematical Contest in Modeling (Data Prize-level recognition)
  * National Endeavor Fellowship (Top 2%)
  * Departmental Scholarship (Top 5%)
  * Startup Competition First Prize Winner

🌍 Leadership & Communication
  * Board Treasurer – Washington Council for Economic and Financial Education
  * Executive Presentations & Stakeholder Alignment
  * Customer Research & Feature Validation
  * Training & Onboarding Program Design
  * Cross-Functional Team Leadership

Publications
======
  <ul>{% for post in site.publications reversed %}
    {% include archive-single-cv.html %}
  {% endfor %}</ul>
<!-- 
Talks
======
  <ul>{% for post in site.talks reversed %}
    {% include archive-single-talk-cv.html  %}
  {% endfor %}</ul> 
--> 
<!--
Teaching
======
  <ul>{% for post in site.teaching reversed %}
    {% include archive-single-cv.html %}
  {% endfor %}</ul> 
-->
  
Service and leadership
======
* Currently signed in to 43 different slack teams
